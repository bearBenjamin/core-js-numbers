...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const distanceOne = Math.sqrt(x1 ** 2 + y1 ** 2);
  const distanceTwo = Math.sqrt(x2 ** 2 + y2 ** 2);
  const cos = scalarProduct / (distanceOne * distanceTwo);
...
function roundToPowerOfTen(num, pow) {
  ...
  const newNum = Math.round(num / tenPower);
  const result = newNum * tenPower;
  return result;
}
